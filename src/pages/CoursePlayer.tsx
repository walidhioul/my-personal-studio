  ...
  return (
    <div className="h-screen flex flex-col bg-background" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <header className="h-14 bg-card border-b border-border flex items-center px-4 gap-3 shrink-0 z-10">
        <Link to={`/courses/${id}`} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0">
            ...
            {currentLesson?.video_url ? (
              ...
              <>
                ...
                <div className="relative text-center z-10">
                  ...
                  {currentLesson && (
                    <p className="text-white/80 text-sm mt-4">{currentLesson.order}. {currentLesson.title}</p>
                  )}
                </div>
              </>
            )}
          ...
          <div className="bg-card border-b border-border px-4 py-3 flex items-center justify-between">
            ...
            {currentLesson && (
              <div className="text-center">
                <p className="text-sm font-medium text-foreground">{currentLesson.order}. {currentLesson.title}</p>
                <p className="text-xs text-muted-foreground flex items-center justify-center gap-1 mt-0.5">
                  <Clock size={12} /> {currentLesson.duration}
                </p>
              </div>
            )}
            <Button variant="default" size="sm" onClick={goNext} disabled={activeLesson === lessons.length - 1} className="gap-1.5">
              {lang === "en" ? "Next" : "التالي"}
          ...
          <div className="flex-1 overflow-auto p-6">
            <h2 className="text-lg font-semibold text-foreground mb-2">{lang === "en" ? "Overview" : "نظرة عامة"}</h2>
            <p className="text-sm text-muted-foreground leading-relaxed">{course.description}</p>
          </div>
        </div>

        {/* Lesson Sidebar */}
        <aside className={`${sidebarOpen ? "w-80" : "w-0"} transition-all duration-300 bg-card border-s border-border shrink-0 overflow-hidden fixed lg:relative inset-y-14 ${isRtl ? "left-0" : "right-0"} lg:inset-y-0 z-20 lg:z-0`}>
              ...
              <div className="p-2">
                {lessons.map((lesson, index) => {
                  ...
                  return (
                    <button
                      key={lesson.id}
                      onClick={() => { setActiveLesson(index); if (window.innerWidth < 1024) setSidebarOpen(false); }}
                      className={`w-full text-start px-3 py-3 rounded-lg flex items-start gap-3 transition-colors mb-0.5 ${isActive ? "bg-primary/10 border border-primary/20" : "hover:bg-muted/50 border border-transparent"}`}
                    >
                      <button onClick={(e) => { e.stopPropagation(); toggleComplete(lesson.id); }} className="mt-0.5 shrink-0">
                        {isCompleted ? <CheckCircle2 size={18} className="text-primary" /> : <Circle size={18} className="text-muted-foreground" />}
                      </button>
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium truncate ${isActive ? "text-primary" : "text-foreground"}`}>{lesson.order}. {lesson.title}</p>
                        <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5"><Play size={10} /> {lesson.duration}</p>
                      </div>
                    </button>
                  );
                })}
