...
const AdminCourses = () => {
  ...
  const submit = async () => {
    ...
    try {
      if (editing) {
        await admin.updateCourse(editing.id, payload);
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage courses, lessons, resources, and quizzes
          </p>
        </div>
        ...
        <div className="mt-8 border rounded-lg bg-card p-6">
          ...
          <Tabs defaultValue="lessons">
            ...
            <TabsContent value="lessons">
              <LessonsPanel courseId={selected.id} />
            </TabsContent>
            <TabsContent value="resources">
              <ResourcesPanel courseId={selected.id} />
            </TabsContent>
            <TabsContent value="quizzes">
              <QuizzesPanel courseId={selected.id} />
            </TabsContent>
          </Tabs>
        </div>
      )}
...
const ResourcesPanel = ({ courseId }: { courseId: number }) => {
  ...
  const submit = async () => {
    try {
      await admin.createResource(courseId, form);
      setForm({ title: "", url: "", type: "pdf" });
      refresh();
      toast.success("Added");
    } catch (e) {
      toast.error((e as Error).message);
...
const QuizzesPanel = ({ courseId }: { courseId: number }) => {
  ...
  const [selectedQuiz, setSelectedQuiz] = useState<admin.AdminQuiz | null>(
    null,
  );
  const refresh = () =>
  ...
  const submit = async () => {
    try {
      await admin.createQuiz(courseId, form);
...
const QuestionsPanel = ({
  ...
  const refresh = () =>
    qc.invalidateQueries({ queryKey: ["admin", "questions", quiz.id] });
